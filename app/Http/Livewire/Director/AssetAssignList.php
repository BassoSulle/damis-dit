...
class AssetAssignList extends Component
{
    use WithPagination;
    ...
    public $director, $headOfDepartment, $headOfSection, $headOfOffice;
    ...
    public function render()
    {
        ...
        foreach ($employeeToAssign as $employee) {
            if (!empty($this->role_id) && ($this->role_id == $employee->role_id)  && ($employee->role->name == 'head of section') && !empty($this->director->department_id) && ($this->director->department_id == $employee->department_id) && !empty($this->section_id) && ($this->section_id == $employee->section_id)) {
                $this->headOfSection_id = $employee->id;
                $this->headOfSection = $employee->first_name . ' ' . $employee->last_name;

            } elseif (!empty($this->role_id) && ($this->role_id == $employee->role_id)  && ($employee->role->name == 'head of office') && !empty($this->director->department_id) && ($this->director->department_id == $employee->department_id) && !empty($this->section_id) && ($this->section_id == $employee->section_id) && !empty($this->room_id) && ($this->room_id == $employee->room_id)) {
        ...
        $this->director = employee::find($director_id);
        ...
        $sections = section::where('department_id', 'like', '%'.$this->director->department_id.'%')->get();
        ...
        ->where('department_id', $this->director->department_id)->whereNull('section_id')
        ->whereNull('room_id')->paginate(10);

        return view('livewire.director.asset-assign-list', [
