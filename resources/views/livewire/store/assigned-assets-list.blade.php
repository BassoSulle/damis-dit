                                        ...
                                        <tr {{ $assignedAsset->id }}>
                                            ...
                                            <td>
                                                @foreach ($employees as $employee)
                                                    @if (!empty($assignedAsset->assigned_by) && ($employee->id == $assignedAsset->assigned_by))
                                                ...
                                                @if (empty($assignedAsset->assigned_by))
                                                    N/A
                                                @endif
                                            </td>
                                            <td>
                                                @foreach ($employees as $employee)
                                                    @if (!empty($assignedAsset->headOfDepartment_id) && empty($assignedAsset->headOfSection_id) && empty($assignedAsset->headOfOffice_id) && ($employee->id == $assignedAsset->headOfDepartment_id))
                                                        <strong>Director: </strong>{{ $employee->first_name . ' ' . $employee->last_name }}

                                                    @elseif (!empty($assignedAsset->headOfDepartment_id) && !empty($assignedAsset->headOfSection_id) && empty($assignedAsset->headOfOffice_id) && ($employee->id == $assignedAsset->headOfDepartment_id))
                                                        <strong>Director: </strong>{{ $employee->first_name . ' ' . $employee->last_name }}

                                                    @elseif (!empty($assignedAsset->headOfDepartment_id) && !empty($assignedAsset->headOfSection_id) && !empty($assignedAsset->headOfOffice_id) && ($employee->id == $assignedAsset->headOfDepartment_id))
                                                        <strong>Director: </strong>{{ $employee->first_name . ' ' . $employee->last_name }}

                                                    @elseif (empty($assignedAsset->headOfDepartment_id) && !empty($assignedAsset->headOfSection_id) && empty($assignedAsset->headOfOffice_id) && ($employee->id == $assignedAsset->headOfSection_id))
                                                        <strong>HOS: </strong>{{ $employee->first_name . ' ' . $employee->last_name }}

                                                    @elseif (empty($assignedAsset->headOfDepartment_id) && !empty($assignedAsset->headOfSection_id) && !empty($assignedAsset->headOfOffice_id) && ($employee->id == $assignedAsset->headOfSection_id))
                                                        <strong>HOS: </strong>{{ $employee->first_name . ' ' . $employee->last_name }}

                                                    @elseif (empty($assignedAsset->headOfDepartment_id) && empty($assignedAsset->headOfSection_id) && !empty($assignedAsset->headOfOffice_id) && ($employee->id == $assignedAsset->headOfOffice_id))
                                                ...
                                                @if (empty($assignedAsset->headOfDepartment_id) && empty($assignedAsset->headOfSection_id) && empty($assignedAsset->headOfOffice_id))
                                                    N/A
                                                @endif
                                            ...
                                            <td>
                                                <button type="button" wire:click="ViewAsset({{ $assignedAsset->asset_id }})" data-toggle="modal" data-target="#ViewAssetModal" class="btn btn-primary"><i class="fa fa-eye"></i></button>
                                            </td>
                                        </tr>
                                        @empty
                                            <tr><td style='color:red; text-align:center;' colspan='10' >No Asset Found!</td></tr>
                                        @endforelse
